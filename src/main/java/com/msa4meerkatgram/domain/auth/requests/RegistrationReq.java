...
public record RegistrationReq(
    ...
    @Schema(description = "닉네임", example = "도플라밍고", nullable = false, requiredMode = Schema.RequiredMode.REQUIRED)
    @NotBlank(message = "닉네임은 필수 항목입니다.")
    @Pattern(regexp = "^[0-9a-zA-Z_]{2,20}$", message = "허용하지 않는 닉네임 양식입니다.")
    String nick,

    @Schema(description = "프로필", example = "http://localhost:8080/files/profiles/", nullable = false, requiredMode = Schema.RequiredMode.REQUIRED)
